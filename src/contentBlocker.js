		...
		'github.com': async () => {
			await page.evaluate(() => {
				...
				localStorage.setItem(
					'cookie-preferences',
					'{"advertising":false,"functionality":true,"performance":false}'
				);
			});
		},
		default: async () => {
			await page.evaluate((selectors) => {
				selectors.forEach((selector) => {
					...
					elements.forEach((element) => {
						if (element && element.style && !element.closest('header') && !element.closest('nav')) {
							element.remove();
						}
					});
				});
...
const setupPage = async (page, url) => {
	try {
		...
		// Handle consent with timeout
		await Promise.race([
			setupConsentHandler(page, url),
			new Promise((_, reject) => setTimeout(() => reject(new Error('Consent handling timeout')), 10000)),
		]).catch((error) => {
			console.warn('Consent handling failed:', error.message);
			// Continue execution even if consent handling fails
		});
	} catch (error) {
